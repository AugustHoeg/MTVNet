...
from SaliencyModel.utils import vis_saliency, grad_abs_norm, grad_abs_norm_2d, grad_abs
...
def main():
    ...
    if opt['input_type'] == "2D":
        ...
        #tensor_lr_backup = tensor_lr.clone()
        #tensor_lr = torch.zeros_like(tensor_lr)
        # tensor_lr[:, 32:-32, 32:-32, 32:-32] = tensor_lr_backup[:, 32:-32, 32:-32, 32:-32] # zero out everything except 64x64x64
        # tensor_lr[:, 48:-48, 48:-48, 48:-48] = tensor_lr_backup[:, 48:-48, 48:-48, 48:-48]  # zero out everything except 32x32x32
        ...
        abs_grad_numpy = grad_abs(grad_numpy)
        grad_mag_sum = abs_grad_numpy[:, :, z_idx_lr].sum()
        ...
        if model_name == "MTVNet" and (opt['netG']['num_levels'] > 1):
            ...
            grad_mag_sum_roi = abs_grad_numpy[crop_idx:-crop_idx, crop_idx:-crop_idx, z_idx_lr].sum()
            ...
            angn_sum = np.sum(abs_normed_grad_numpy[crop_idx:-crop_idx, crop_idx:-crop_idx, crop_idx:-crop_idx], axis=2)
            # angn_mean = (angn_mean - np.min(angn_mean)) / (np.max(angn_mean) - np.min(angn_mean))
            saliency_image_abs_mean = vis_saliency(angn_sum, zoomin=up_factor)
            ...
            # pad gradient image up to same size as MTVNet for fair comparison
            abs_normed_grad_numpy = pad
            ...
            angn_sum = np.sum(abs_normed_grad_numpy, axis=2)
            # angn_mean = (angn_mean - np.min(angn_mean)) / (np.max(angn_mean) - np.min(angn_mean))
            saliency_image_abs_mean = vis_saliency(angn_sum, zoomin=up_factor)
            blend_abs_and_hr = cv2_to_pil(pil_to_cv2(saliency_image_abs) * (1.0 - alpha) + pil_to_cv2(pil_hr) * alpha)
