...
def load_EDDSR(load_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    up_factor = 4

    generator = EDDSR_xs(up_factor=up_factor).to(device)

    load_network(load_path, generator, strict=False, param_key='params')
...
def test():
    ...
    # Set volumetric image size
    patch_size = 48
    # Set upscaling factor, either 2x, 3x or 4x
    up_factor = 1

    # Generate test image of random numbers
    ...
    #load_network("EDDSR_Synthetic_2022_QIM_52_Bone_4x.h5", generator, strict=False, param_key='params')
