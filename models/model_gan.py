...
from loss_functions.loss_functions_simple import VGGLoss, VGGLoss3D, GradientLoss3D, TotalVariationLoss3D, TextureLoss3D, \
    StructureLoss3D, LaplacianLoss3D, compute_generator_loss, compute_discriminator_loss
...
class ModelGAN(ModelBase):
    ...
    # # real
    # l_d_real = 0.5 * F.binary_cross_entropy_with_logits(prop_real - torch.mean(prop_fake, 0, True), True)
    # l_d_real.backward()
    # # fake
    # pred_d_fake = self.netD(self.E.detach())
    # l_d_fake = 0.5 * F.binary_cross_entropy_with_logits(pred_d_fake - torch.mean(prop_real.detach(), 0, True),
    #                                                     False)
    # l_d_fake.backward()
    ...
    def optimize_parameters(self, current_step, update=False):
        ...
        if self.opt['model'] == "gan":
            ...
            self.dis_loss.backward()  # backward-pass to compute gradients

        elif self.opt['model'] == "ragan":
            self.prop_fake = self.netD_forward(self.E).detach()  # 2) fake data, detach to avoid backprop to G.
            dis_loss_real = 0.5 * self.loss_fn_dict['BCE_Logistic'](self.prop_real - torch.mean(self.prop_fake, dim=0), torch.ones_like(self.prop_fake))
            dis_loss_real = dis_loss_real / self.num_accum_steps_D  # Scale loss by number of accumulation steps
            self.dis_loss = dis_loss_real  # Record D loss on reals
            dis_loss_real.backward()

            self.prop_fake = self.netD_forward(self.E.detach())
            dis_loss_fake = 0.5 * self.loss_fn_dict['BCE_Logistic'](self.prop_fake - torch.mean(self.prop_real.detach(), dim=0),  torch.zeros_like(self.prop_fake))
            dis_loss_fake = dis_loss_fake / self.num_accum_steps_D  # Scale loss by number of accumulation steps
            self.dis_loss += dis_loss_fake  # Record D loss on fakes
            dis_loss_fake.backward()

        self.D_train_loss += self.dis_loss  # Add discriminator training loss to total loss

        if ((self.D_accum_count + 1) % self.num_accum_steps_D) == 0 or update:  # Gradient acculumation
            # ------------------------------------
            # clip_grad on D
            # ------------------------------------
        ...
        if self.opt['model'] == 'ragan':
            self.prop_real = self.netD_forward(self.H).detach()  # Compute prop_fake again, as the value was discarded by backward

        self.gen_loss = compute_generator_loss(self.H, self.E, self.loss_fn_dict, self.loss_val_dict, self.prop_real, self.prop_fake, self.opt['model'], self.device)
    ...
    def optimize_parameters_amp(self, current_step, update=False):
        ...
        with torch.cuda.amp.autocast(dtype=self.mixed_precision):
            self.prop_fake = self.netD_forward(self.E)  # Compute prop_fake again, as the value was discarded by backward
            self.gen_loss = compute_generator_loss(self.H, self.E, self.loss_fn_dict, self.loss_val_dict, self.prop_fake, self.opt['model'], self.device)
            self.gen_loss = self.gen_loss / self.num_accum_steps_G  # Scale loss by number of accumulation steps
