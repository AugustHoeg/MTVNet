...
class Dataset_2022_QIM_52_Bone():
    ...
    def get_file_paths(self, synthetic=False):
    ...
    def get_transforms(self, mode, synthetic=False):
        ...
        if self.opt['datasets']["dataset_type"] == "MonaiSmartCacheDataset" or self.opt['datasets']["dataset_type"] == "CacheDataset":
            data_trans = Femur_transforms_cachableV2(self.opt, mode, synthetic)
        else:
            data_trans = Femur_transforms(self.opt, mode, synthetic)
        transforms = data_trans.get_transforms()
        return transforms

    def get_baseline_transforms(self, mode, synthetic=False):

        data_trans = Femur_baseline_transformsV2(self.opt, mode, synthetic)
        transforms = data_trans.get_transforms()
        return transforms
