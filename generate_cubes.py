...
def save_image_cubes(model, opt, number_of_cubes, image_dir,  train_loader, test_loader, print_status=True):
    ...
    while current_step < number_of_cubes:
        ...
        for batch_idx, test_batch in enumerate(test_loader):
            ...
            np.save(os.path.join(os.path.join(image_dir, "HR/"),  f"cube_{idx:03d}.npy"), HR_cube)
            np.save(os.path.join(os.path.join(image_dir, "LR/"), f"cube_32_{idx:03d}.npy"), LR_cube_32)
            np.save(os.path.join(os.path.join(image_dir, "LR/"), f"cube_64_{idx:03d}.npy"), LR_cube_64)
            np.save(os.path.join(os.path.join(image_dir, "LR/"), f"cube_128_{idx:03d}.npy"), LR_cube_128)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(image_dir, "HR/")):
        os.makedirs(os.path.join(image_dir, "HR/"))
    if not os.path.exists(os.path.join(image_dir, "LR/")):
        os.makedirs(os.path.join(image_dir, "LR/"))
